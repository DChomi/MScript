#!/usr/bin/env python3
"""
Vless.py - Vless 协议部署模块
继承 MihomoBase 基类,实现 Vless 协议的具体部署
...
class VlessInstaller(MihomoBase):
    """Vless 协议安装器"""

    def __init__(self):
        super().__init__()
        self.protocol_name = "Vless"
    ...
    def get_deployment_config(self):
        ...
        return {
            ...
            'short_id': short_id if use_reality else None  # 添加这一行
        }
    ...
    def print_final_info(self, config):
        ...
        print(f"📋 Vless ({mode_name} 模式) 客户端配置:\n")
        ...
        if config['use_reality']:
            ...
            print(f"- name: Vless|Reality|{config['fake_domain']}")
            ...
            print(f"  servername: {sni_display}")
            ...
            print(f"- name: Vless|TLS|{config['domain']}")
            print(f"  server: {server_display}")
            print(f"  type: vless")
            print(f"  port: {config['port']}")
            print(f"  uuid: {config['uuid']}")
            ...
            print(f"  tls: true")
            print(f"  flow: xtls-rprx-vision")
            print(f"  servername: {config['domain']}")
        ...
        print("---[ Compact 格式 ]---")
        if config['use_reality']:
            compact = f'- {{name: "Vless|Reality|{config["fake_domain"]}", type: vless, server: {public_ip}, port: {config["port"]}, uuid: {config["uuid"]}, network: tcp, udp: true, tls: true, flow: xtls-rprx-vision, servername: {config["fake_domain"]}, reality-opts: {{public-key: {config["public_key"]}, short-id: {config["short_id"]}}}, client-fingerprint: chrome}}'
        else:
            skip_verify = "true" if config['use_self_signed'] else "false"
            compact = f'- {{name: "Vless|TLS|{config["domain"]}", type: vless, server: {config["domain"]}, port: {config["port"]}, uuid: {config["uuid"]}, network: tcp, udp: true, tls: true, flow: xtls-rprx-vision, servername: {config["domain"]}, skip-cert-verify: {skip_verify}, client-fingerprint: chrome}}'
        print(f"{compact}\n")

        # URI 格式
        print("---[ URI 格式 ]---")
        if config['use_reality']:
            uri = f"vless://{config['uuid']}@{public_ip}:{config['port']}?encryption=none&flow=xtls-rprx-vision&security=reality&sni={config['fake_domain']}&fp=chrome&pbk={config['public_key']}&sid={config['short_id']}&type=tcp&headerType=none#Vless|Reality|{config['fake_domain']}"
        else:
            insecure_flag = "1" if config['use_self_signed'] else "0"
            uri = f"vless://{config['uuid']}@{config['domain']}:{config['port']}?encryption=none&flow=xtls-rprx-vision&security=tls&sni={config['domain']}&fp=chrome&type=tcp&headerType=none&insecure={insecure_flag}#Vless|TLS|{config['domain']}"
        print(f"{uri}\n")
